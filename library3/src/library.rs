    ...
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        if let Some(year) = self.year {
            write!(
                f,
                "ID: {}\nTitle: {}\nAuthor: {}\n{}Year: {}\nAvailable: {}\nKeywords: {:?}",
                self.id, self.title, self.author, &self.media_type, year, self.available, self.keywords
            )
        } else {
            write!(
                f,
...
impl MediaType {
    ...
    pub fn change_isbn10(&mut self, isbn: u64) -> Result<(), ErrorKind> {
        match self {
            ...
            media_type => Err(ErrorKind::MediaDoesntHaveIsbn(media_type.to_string())),
        }
    }
    ...
    pub fn add(&mut self, media: Media) -> Result<(), ErrorKind> {
        if self.contains(&media) {
            match media.media_type {
                ...
                _ => return Err(ErrorKind::MediaAlreadyExists(media.media_type.to_string())),
            }
        }
